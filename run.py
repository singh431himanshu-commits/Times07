...
def search_hd_images(query, count=5):
    ...
    try:
        time.sleep(2)
        search_queries = [f'"{query}" official news photo', f'"{query}" India news', f'"{query}" latest event', query]
        with DDGS() as ddgs:
            results = []
            for q in search_queries:
                try:
                    temp = list(ddgs.images(query=q, region="in-en", safesearch="on", size="Large", type_image="photo", layout="Wide", max_results=25))
                    results.extend(temp)
                    results = list({item.get("image"): item for item in results if item.get("image")}.values())
                    if len(results) >= 80: break
                except: pass
            
            for i, res in enumerate(results):
                if "image" not in res: continue
                raw_url = res["image"]
                ...
                if any(x in title for x in ["youtube", "thumbnail", "logo", "poster", "wallpaper", "stock", "vector", "illustration"]): continue
    ...
    for key in available_keys:
        for model in models:
            try:
                ...
                search_keyword = re.sub(r"\|.*|\b20\d{2}\b", "", raw_title)
                data["image_options"] = search_hd_images(data.get("image_keyword", search_keyword), count=5)
...
def run_bot():
    ...
    # 1. पहले Google Trends से सिर्फ 1 ताज़ा खबर लेगा
    for topic in fetch_google_trends()[:1]:
        for entry in feedparser.parse(f"https://news.google.com/rss/search?q={urllib.parse.quote(topic)}&hl=hi&gl=IN&ceid=IN:hi").entries[:1]:
...
if __name__ == "__main__":
    ...
    while True:
        try: run_bot()
        except Exception as e: error_logger.error(f"Main loop crash: {e}")
        os.system('git add -A')
